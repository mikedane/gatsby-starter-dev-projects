...
module.exports = {
  setupFilesAfterEnv: [
    path.resolve(__dirname, './jest-configs/setup-test-env.ts'),
  ],
  transform: {
    // "^.+\\.(tsx?|jsx?)$": "ts-jest",
    '\\.svg': '<rootDir>/jest-configs/__mocks__/svg-transform.ts',
    '^.+\\.(tsx?|jsx?)$': `<rootDir>/jest-configs/jest-preprocess.ts`,
  },
  moduleNameMapper: {
    // "\\.svg": `./jest-configs/__mocks__/file-mocks.js`,
    '\\.svg': `<rootDir>/jest-configs/__mocks__/svgTransform.ts`,
